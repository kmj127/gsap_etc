...
document.querySelectorAll('.tit_card').forEach((card) => {
  ...
  card.addEventListener('mousemove', (e) => {
    ...
    layer.style.transform = `translateZ(0) rotateX(${rx}deg) rotate(${ry}deg)`;

    card.addEventListener('mouseleaver', () => {
      layer.style.transform = `translateZ(0) rotateX(0deg) rotate(0deg)`;
    })
  });
})
...
//sticky 박스의 이미지를 바로 바꾸면 딱딱하게 바뀌어서 살짝 줄였다가 다시 키우며 교체
function changeBg(imgUrl) {
  ...
  gsap.to(stickyBox, {
    ...
    onComplete: () => {
      ...
      gsap.to(stickyBox, { opacity: 1, scale1, duration: 0.45 });
    }
  })
}
...
photos.forEach((photo, index) => {
  ...
  pinTl.add(() => {
    ...
    //0.4초 뒤에 glitch 클래스를 제거해 효과를 끝낸다.
